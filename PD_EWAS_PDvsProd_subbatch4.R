...
pheno <- pheno[pheno$DX_new_char != "HC",]

##### Prod vs PD
setwd("/dartfs/rc/lab/S/SalasLab/PD/Processed_data/Feb2023/EWAS")
try(dir.create("PDvsProd"))
setwd("PDvsProd")
tmp_cols <- c(Prod="#3971B5", PD="#C56124")

my_combos_to_test <- list(Unadj = NULL,
                          NLR = "NLR",
                          Many = c("Neu","Mono","Bnv","CD4mem","Eos"))

CHRS <- c("chr11", "chr12","chr5","chr15", "chr21")
for (CHR in CHRS){
  setwd("/dartfs/rc/lab/S/SalasLab/PD/Processed_data/Feb2023/EWAS/PDvsProd/")
  try(dir.create(CHR))
  setwd(CHR)
  ...
  for (timept in c("BL","V04","V06","V08")){
    ...
    DeconvoEWAS(df=pheno2, # data frame with all data, colnames should match variable names within reg.model, cell.types, rownames should match colnames of betas 
                ...
                base_model="DX_new_char + weird_batch + rundate_binary + BL_age + is.Female", # the right hand side of the regression model equation
                ...
                ref_group="Prod", # what is the reference group
                case_group="PD", # what is the group to compare 
                save_output_csv = TRUE, # save the results in csv table
                save_output_pdf = TRUE, # save a pdf version of the plot
                file_output_prefix = paste0(CHR,"_EWAS_",timept,"_PDvsProd"), # string to attach to saved file names
  ...
  DeconvoEWAS(df=pheno2, # data frame with all data, colnames should match variable names within reg.model, cell.types, rownames should match colnames of betas 
              betas=tmp_betas, # beta matrix for dataset
              base_model="DX_new_char + weird_batch + rundate_binary + BL_age + is.Female + Sample_Time", # the right hand side of the regression model equation
              user_p_adjust_method = "fdr", # adjustment method for multiple testing
              cell_types=cell_types, # list of cell types 
              cell_combos_to_test = my_combos_to_test, # named list of vectors containing names of cell types to try, each entry of list will be one model, leave one entry as NULL to do unadj
              model_type = "lmer",
  ...
  DeconvoEWAS(df=pheno2, # data frame with all data, colnames should match variable names within reg.model, cell.types, rownames should match colnames of betas 
              betas=tmp_betas, # beta matrix for dataset
              base_model="DX_new_char*Sample_Time + weird_batch + rundate_binary + BL_age + is.Female", # the right hand side of the regression model equation
              user_p_adjust_method = "fdr", # adjustment method for multiple testing
              cell_types=cell_types, # list of cell types 
              cell_combos_to_test = my_combos_to_test, # named list of vectors containing names of cell types to try, each entry of list will be one model, leave one entry as NULL to do unadj
              model_type = "lmer",
