...
pheno$rundate_bindary <- ifelse(pheno$run_date_group %in% c("Jan_2021", "Feb_2021", "March_2021"),1,0)
...
for (CHR in names(cpg_chr_names)){
  ...
  setwd("/dartfs/rc/lab/S/SalasLab/PD/Processed_data/Feb2023/EWAS/PDvsHC/")
  try(dir.create(CHR))
  setwd(CHR)
  ...
  for (timept in c("BL","V04","V06","V08")){
    ...
    DeconvoEWAS(df=pheno2, # data frame with all data, colnames should match variable names within reg.model, cell.types, rownames should match colnames of betas 
      ...
      base_model="DX_new_char + weird_batch + rundate_bindary + BL_age + is.Female", # the right hand side of the regression model equation
      ...
      file_output_prefix = paste0(CHR,"_EWAS_",timept,"_fulldata_newbatch"), # string to attach to saved file names
  ...
  DeconvoEWAS(df=pheno2, # data frame with all data, colnames should match variable names within reg.model, cell.types, rownames should match colnames of betas 
              betas=tmp_betas, # beta matrix for dataset
              base_model="DX_new_char + weird_batch + rundate_bindary + BL_age + is.Female + Sample_Time", # the right hand side of the regression model equation
              user_p_adjust_method = "fdr", # adjustment method for multiple testing
              cell_types=cell_types, # list of cell types 
              cell_combos_to_test = my_combos_to_test, # named list of vectors containing names of cell types to try, each entry of list will be one model, leave one entry as NULL to do unadj
              model_type = "lmer",
  ...
  DeconvoEWAS(df=pheno2, # data frame with all data, colnames should match variable names within reg.model, cell.types, rownames should match colnames of betas 
              betas=tmp_betas, # beta matrix for dataset
              base_model="DX_new_char*Sample_Time + rundate_bindary + weird_batch + BL_age + is.Female", # the right hand side of the regression model equation
              user_p_adjust_method = "fdr", # adjustment method for multiple testing
              cell_types=cell_types, # list of cell types 
              cell_combos_to_test = my_combos_to_test, # named list of vectors containing names of cell types to try, each entry of list will be one model, leave one entry as NULL to do unadj
              model_type = "lmer",
